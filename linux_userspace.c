...
int8_t user_i2c_read(uint8_t reg_addr, uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    //another way
    ret = i2c_smbus_read_byte_data(id.fd, reg_addr);
    *data = ret;
    */
    ret = i2c_smbus_read_i2c_block_data(id.fd, reg_addr, len, data);
    if(ret < 0)
    {
        fprintf(stderr, "Failed to red reg_addr=%#x ret=%d\n", reg_addr, ret);
        return 1;
    }
    return 0;
}
...
int8_t user_i2c_write(uint8_t reg_addr, const uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    if(len != 1)
    {
        fprintf(stderr, "Failed to write reg_addr=%#x, data len=%d not supported\n", reg_addr, len);
        return BME280_E_INVALID_LEN;
    }
    value = *data;
    ret = i2c_smbus_write_byte_data(id.fd, reg_addr, value);
    if(ret < 0)
    {
        fprintf(stderr, "Failed to write reg_addr=%#x ret=%d, value=%#x\n", reg_addr, ret, value);
        return BME280_E_COMM_FAIL;
    }
    /*
    uint8_t *buf;
